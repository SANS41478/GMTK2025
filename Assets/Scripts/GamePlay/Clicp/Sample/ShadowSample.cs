...
namespace GamePlay
{
    ...
    {
        private readonly List<SamplePointObj> cenplePoints = new List<SamplePointObj>();
        ...
        public void Init( IList<IList<IMoveEventData>> datas, GameObject samplePointPrefab )
        {
            ...
            {
                GameObject obj = Instantiate(samplePointPrefab, transform);
        ...
        public void SamplePoints(Vector2Int startPoint)
        {
            ...
            for (int i = 0; i < datas.Count; i++)
            {
                ...
                foreach (IMoveEventData data in datas[i])
                {
        ...
        {
            bool mark = false;
            foreach (IList<IMoveEventData> dataLis in datas)
            {
                if (dataLis.Count > 0)
                    foreach (IMoveEventData te in dataLis)
                    {
                        Vector2Int  temp = startPoint + te.endPosition;
